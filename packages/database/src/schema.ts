...
export const countries = pgTable('countries', {
  ...
  index('countries_active_name_idx').on(table.is_active, table.name)
]);

export const cities = pgTable('cities', {
  id: uuid('id').defaultRandom().primaryKey(),
  country_id: uuid('country_id').notNull().references(() => countries.id, { onDelete: 'cascade' }),
  name: varchar('name', { length: 120 }).notNull(),
...
export const businessProfiles = pgTable('business_profiles', {
  ...
  country_id: uuid('country_id').notNull().references(() => countries.id, { onDelete: 'restrict' }),
  city_id: uuid('city_id').notNull().references(() => cities.id, { onDelete: 'restrict' }),
  ...
  index('bp_status_featured_cat_idx').on(table.status, table.featured_top, table.featured_recommended, table.category_id),
  index('bp_status_loc_cat_pub_idx').on(table.status, table.country_id, table.city_id, table.category_id, table.published_at)
]);

export const subscriptions = pgTable('subscriptions', {
  id: uuid('id').defaultRandom().primaryKey(),
  user_id: uuid('user_id').references(() => users.id, { onDelete: 'set null' }),
  business_profile_id: uuid('business_profile_id').references(() => businessProfiles.id, { onDelete: 'set null' }),
  ...
  index('subs_user_kind_status_end_idx').on(table.user_id, table.kind, table.status, table.current_period_end),
  index('subs_bp_kind_status_end_idx').on(table.business_profile_id, table.kind, table.status, table.current_period_end)
]);

export const businessIntroductions = pgTable('business_introductions', {
  id: uuid('id').defaultRandom().primaryKey(),
  requester_user_id: uuid('requester_user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  requester_business_id: uuid('requester_business_id').references(() => businessProfiles.id, { onDelete: 'cascade' }),
  target_business_id: uuid('target_business_id').notNull().references(() => businessProfiles.id, { onDelete: 'cascade' }),
  ...
  index('intro_req_status_created_idx').on(table.requester_user_id, table.status, table.created_at),
  index('intro_target_status_created_idx').on(table.target_business_id, table.status, table.created_at)
]);
...
export const admin2fa = pgTable('admin_2fa', {
  id: uuid('id').defaultRandom().primaryKey(),
  admin_user_id: uuid('admin_user_id').notNull().unique().references(() => adminUsers.id, { onDelete: 'cascade' }),
...
export const adminSessions = pgTable('admin_sessions', {
  id: uuid('id').defaultRandom().primaryKey(),
  admin_user_id: uuid('admin_user_id').notNull().references(() => adminUsers.id, { onDelete: 'cascade' }),
...
export const auditLogs = pgTable('audit_logs', {
  id: uuid('id').defaultRandom().primaryKey(),
  actor_staff_id: uuid('actor_staff_id').references(() => adminUsers.id, { onDelete: 'set null' }),
...
export const adminConfigs = pgTable('admin_config', {
  id: uuid('id').defaultRandom().primaryKey(),
  key: varchar('key', { length: 120 }).notNull().unique(),
  value: json('value').notNull(),
  description: text('description'),
  updated_by_staff_id: uuid('updated_by_staff_id').references(() => adminUsers.id, { onDelete: 'set null' }),
  created_at: timestamp('created_at').defaultNow().notNull(),
  updated_at: timestamp('updated_at').defaultNow().notNull(),
}, (table) => [
  index('admin_config_key_idx').on(table.key)
]);
