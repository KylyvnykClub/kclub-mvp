...
describe('centralized constants', () => {
  ...
  test('matches the staff permission matrix from the spec', () => {
    ...
    expect(STAFF_ROLE_PERMISSIONS.SUPPORT).toEqual([
    ...
    expect(STAFF_ROLE_PERMISSIONS.SUPPORT).not.toContain(STAFF_PERMISSIONS.USERS_READ);
  });
...
describe('route contracts', () => {
  test('keeps member and admin route bases stable', () => {
    ...
    expect(MEMBER_API_ROUTES.BUSINESS_RESERVE_REVIEW).toBe('/api/v1/businesses/reserve-review');
