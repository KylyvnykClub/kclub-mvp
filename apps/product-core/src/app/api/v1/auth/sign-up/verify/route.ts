...
import { memberSignUpVerifySchema, parseWithValidation } from '@kclub/validation';
...
export async function POST(request: NextRequest): Promise<Response> {
  try {
    ...
    const parsed = parseWithValidation(memberSignUpVerifySchema, await request.json());
    if (!parsed.success) {
      return jsonError(
        {
    ...
    const { supabaseUserId } = await verifyMemberSignUp(
