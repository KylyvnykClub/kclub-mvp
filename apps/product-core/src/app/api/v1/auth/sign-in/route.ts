...
import { memberPasswordSignInSchema, parseWithValidation } from '@kclub/validation';
...
export async function POST(request: NextRequest): Promise<Response> {
  try {
    ...
    const parsed = parseWithValidation(memberPasswordSignInSchema, await request.json());
    if (!parsed.success) {
      return jsonError(
        {
    ...
    const { supabaseUserId } = await signInMemberWithPassword(
